        ...
        return new Promise((resolve, reject) => {
            ...
            return fs.open(`${this.directory}/${file.id}`, 'w', (err, fd) => {
                ...
                return fs.close(fd, (exception) => {
                    ...
                    this.emit(EVENTS.EVENT_FILE_CREATED, { req: req, file: file });
                    return resolve(file);
                });
            });
        });
    ...
    write(req, file_id, offset) {
        return new Promise((resolve, reject) => {
            ...
            req.on('data', (buffer) => {
                ...
                if (new_offset>config.upload_length) {
                    log('[FileStore] write: received more than upload_length !');
                    reject(ERRORS.UNKNOWN_ERROR);
                }
            });

            req.on('close', () => {
                log('[FileStore] write: request was closed, ending stream');
                stream.end();
            });
            ...
            return req.pipe(stream).on('finish', () => {
                ...
                if (config && parseInt(config.upload_length, 10) === offset) {
                    this.emit(EVENTS.EVENT_UPLOAD_COMPLETE, { req: req, file: config });
                }
                resolve(offset);
            });
        });
